...
describe('Booking & Wallet Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const ceremony = await Ceremony.create({
            ...
            ritualSteps: [
                { stepNumber: 1, title: 'Introduction', description: 'Start' }
            ],
            requirements: { 
                materials: [],
    ...
    test('Test A: Happy Path - Complete Booking Flow', async () => {
        ...
        const transaction = await Transaction.findOne({ bookingId: bookingId, type: 'credit_for_booking' });
    ...
    test('Test C: Devotee Cancels Post-Payment', async () => {
        // Arrange
        const bookingRes = await request(app).post('/api/bookings').set('Authorization', `Bearer ${devoteeToken}`).send({
            priestId: priestId, ceremonyType: 'Ganesh Puja', date: new Date(Date.now() + 86400000).toISOString(),
            startTime: '10:00', endTime: '11:00', location: { address: 'Test Street 123', city: 'Bangalore' }
        });
        const bookingId = bookingRes.body.data._id;
        await request(app).put(`/api/bookings/${bookingId}/status`).set('Authorization', `Bearer ${priestToken}`).send({ status: 'confirmed' });
