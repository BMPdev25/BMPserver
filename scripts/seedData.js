...
const seedData = async () => {
  try {
    ...
    if (ceremonyCount === 0) {
      console.log(
        'WARNING: No ceremonies found. Please run full_seed.js first or ensure ceremonies are populated.'
      );
    ...
    const priests = [
      ...
      },
      {
        ...
        city: 'Delhi',
        services: [{ ceremonyId: satyanarayan._id, price: 2500, durationMinutes: 120 }],
      },
    ];
    ...
    for (const p of priests) {
      ...
      if (!user) {
        ...
        await PriestProfile.create({
          ...
          location: {
            type: 'Point',
            coordinates: p.city === 'Hyderabad' ? [78.4867, 17.385] : [77.209, 28.6139],
          },
          isVerified: true,
        });
