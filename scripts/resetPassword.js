...
const updatePassword = async () => {
    try {
        ...
        // List all priests
        const priests = await mongoose.connection.collection('users')
            .find({ userType: 'priest' }, { projection: { name: 1, email: 1, phone: 1 } })
            .toArray();
        console.log('=== PRIEST ACCOUNTS ===');
        priests.forEach(p => console.log(`Name: ${p.name} | Email: ${p.email} | Phone: ${p.phone} | ID: ${p._id}`));
        console.log('=======================');
