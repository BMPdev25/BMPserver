...
const createBookingRules = [
  body('priestId')
    .notEmpty()
    .withMessage('Priest ID is required')
    .isMongoId()
    .withMessage('Invalid priest ID'),
  body('ceremonyType')
    .trim()
    .notEmpty()
    .withMessage('Ceremony type is required')
    .isLength({ max: 100 })
    .withMessage('Ceremony type too long'),
  body('date')
    .notEmpty()
...
const createInstantBookingRules = [
  body('ceremonyId')
    .notEmpty()
    .withMessage('Ceremony ID is required')
    .isMongoId()
    .withMessage('Invalid ceremony ID'),
  body('preferredPriestId')
    .optional({ nullable: true })
    .isMongoId()
    .withMessage('Invalid preferred priest ID'),
...
const verifyPaymentRules = [
  ...
  body('rzpPaymentId').notEmpty().withMessage('Payment ID is required'),
  body('rzpOrderId').notEmpty().withMessage('Order ID is required'),
  body('rzpSignature').notEmpty().withMessage('Signature is required'),
];
