...
const submitRatingRules = [
  ...
  body('categories.knowledge')
    .notEmpty()
    .withMessage('Knowledge rating is required')
    .isInt({ min: 1, max: 5 }),
  body('categories.behavior')
    .notEmpty()
    .withMessage('Behavior rating is required')
    .isInt({ min: 1, max: 5 }),
  body('categories.overall')
    .notEmpty()
    .withMessage('Overall rating is required')
    .isInt({ min: 1, max: 5 }),
  body('ceremonyType').notEmpty().withMessage('Ceremony type is required'),
  body('ceremonyDate').notEmpty().withMessage('Ceremony date is required'),
  body('review')
    .optional()
    .isLength({ max: 500 })
